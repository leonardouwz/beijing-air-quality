...
# WDIR_sin/cos: codificación sine/cosine de dirección del viento (variable circular)
...
# Mapa de dirección del viento (UCI PRSA: string categórico → código 0-15)
# 16 direcciones de brújula + 'cv' (calmo/variable) tratado como sin/cos = 0
WDIR_MAP = {
    'N':0, 'NNE':1, 'NE':2, 'ENE':3, 'E':4, 'ESE':5, 'SE':6, 'SSE':7,
    'S':8, 'SSW':9, 'SW':10,'WSW':11,'W':12,'WNW':13,'NW':14,'NNW':15,
    'cv':16,  # calmo/variable → sin=0, cos=1 (apunta a Norte como neutro)
}


def _encode_wdir_categorical(wd_series) -> tuple:
    """
    Codifica columna 'wd' (string UCI) a WDIR_sin y WDIR_cos.
    'cv' (calmo) → código 16 → sin=0, cos=1 (sin dirección dominante).
    Retorna (sin_series, cos_series).
    """
    codes = wd_series.map(WDIR_MAP).fillna(16)  # NaN → calmo
    angle = 2 * np.pi * codes / 16
    return np.sin(angle), np.cos(angle)


def _encode_wdir_degrees(wd_degrees) -> tuple:
    """
    Codifica columna de dirección en grados (0-360, Open-Meteo) a sin/cos.
    Retorna (sin_series, cos_series).
    """
    angle = 2 * np.pi * wd_degrees / 360.0
    return np.sin(angle), np.cos(angle)
...
def load_historical_data(base_path: str = "../datasets/2013-2017/PRSA_Data_20130301-20170228",
    ...
    for station in stations:
        ...
        df = df.rename(columns={"station": "region", "DEWP": "DEW"})
        ...
        # Convertir features numéricas (excluir las derivadas WDIR_sin/cos)
        BASE_FEATURES = ["PM2.5","PM10","SO2","NO2","CO","O3","TEMP","PRES","DEW","WSPM"]
        for col in BASE_FEATURES:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")

        # Codificar dirección del viento → WDIR_sin, WDIR_cos
        if "wd" in df.columns:
            df["WDIR_sin"], df["WDIR_cos"] = _encode_wdir_categorical(df["wd"])
        else:
            df["WDIR_sin"] = 0.0
            df["WDIR_cos"] = 1.0  # sin dirección → neutro
...
def load_current_data(path: str = "../datasets/2022-2026/air_quality_historical.csv") -> pd.DataFrame:
    ...
    # Codificar dirección del viento (Open-Meteo: grados numéricos 0-360)
    if "wd" in df.columns:
        wd_num = pd.to_numeric(df["wd"], errors="coerce").fillna(0.0)
        df["WDIR_sin"], df["WDIR_cos"] = _encode_wdir_degrees(wd_num)
    else:
        df["WDIR_sin"] = 0.0
        df["WDIR_cos"] = 1.0
