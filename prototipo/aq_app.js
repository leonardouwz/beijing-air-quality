...
(function () {
  ...
  const LOAD1 = "#5bb0ff";                // color de PC1 en los gráficos de cargas

  // ── Etiquetas legibles de las 10 variables (con unidades) ──
  const FEAT_LABELS = {
  ...
  const FEAT_SHORT = { "DEW": "DEWP" };    // alias corto para ejes/etiquetas
  const flabel = f => FEAT_LABELS[f] || f;
  const fshort = f => FEAT_SHORT[f] || f;
  const fmtNum = v => (Math.abs(v) >= 100 ? v.toFixed(0) : v.toFixed(1));

  // ── AQI (US EPA) — sub-índice lineal por tramos ──
  //   AQI = (Ihi-Ilo)/(Chi-Clo)·(C-Clo)+Ilo, y el AQI final = MÁX de los subíndices.
  //   Breakpoints en µg/m³ (24 h) para PM2.5 y PM10: son las dos variables del vector
  //   cuyas unidades coinciden con la tabla EPA, así el cálculo es exacto sin convertir
  //   a ppb/ppm. Los gaseosos (O3/NO2/SO2/CO) usan ppb/ppm en EPA → se dejan como
  //   extensión (requieren conversión) para no introducir error.
  const AQI_BP = {
  ...
  // Bandas por valor de AQI (nombre + color), según la tabla del plan.
  const AQI_BANDS = [
  ...
  function aqiOf(i) {                       // máximo subíndice disponible (PM2.5, PM10)
    let mx = 0;
  ...
  let AQIv, cityAqiAvg = 0, featMeanAll;        // AQI por registro, AQI medio ciudad, medias norm.
  ...
  // Pre-cálculo (una vez por dataset): AQI de cada registro + medias normalizadas por
  // feature. Mantiene el redibujo del brushing en O(seleccionados) en vez de O(N).
  ...
  // Se recalculan al cambiar de dataset (setupA → renderLoadings) y al redimensionar.
  function renderLoadings() {
    if (!pcs) return;
    const v1 = pcs[0].vec, v2 = pcs[1].vec;
    let mx = 0; for (let j = 0; j < FEAT.length; j++) mx = Math.max(mx, Math.abs(v1[j]), Math.abs(v2[j]));
    mx = mx || 1;

    // ── Lollipop horizontal con signo (svgLoad1): una "piruleta" por variable y PC ──
    (function () {
    ...
    // ── Polígono comparativo PC1 vs PC2 (svgLoad2): dos líneas sobre el eje X de variables ──
    (function () {
  ...
  // ── Badge de AQI en la barra superior (enlazado al brushing) ──
  ...
  // ── Toolbox de variable + estadísticos univariados (B/C) ──
  ...
  function drawHist(cfg) {
    ...
    sel.exit().remove();
    // Estadístico de la selección añadido al univariado.
